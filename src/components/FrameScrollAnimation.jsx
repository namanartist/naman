...
const FrameScrollAnimation = ({ frameCount = 240 }) => {
  ...
  // Floating motion
  const y = useTransform(smoothProgress, [0, 1], ["0%", isMobile ? "-5%" : "-10%"]);
  
  // Conditionally disable rotation on mobile for a cleaner, redesigned look
  const rotateXDesktop = useTransform(smoothProgress, [0, 0.5, 1], [0, 15, 0]);
  const rotateYDesktop = useTransform(smoothProgress, [0, 0.3, 0.7, 1], [0, -10, 10, 0]);
  const rotateX = isMobile ? 0 : rotateXDesktop;
  const rotateY = isMobile ? 0 : rotateYDesktop;

  const scaleDesktop = useTransform(smoothProgress, [0, 0.8, 0.95], [1, 1.05, 1.2]);
  const scaleMobile = useTransform(smoothProgress, [0, 0.8, 0.95], [1, 1.02, 1.1]);
  const scale = isMobile ? scaleMobile : scaleDesktop;
  
  // Depth effect (Z-axis translation)
  const zDesktop = useTransform(smoothProgress, [0, 1], [0, 100]);
  const zMobile = useTransform(smoothProgress, [0, 1], [0, 30]);
  const z = isMobile ? zMobile : zDesktop;
  ...
  return (
    <div ref={containerRef} className="relative h-[600vh] bg-gray-50 dark:bg-[#020202] transition-colors duration-500">
      <div className="sticky top-0 left-0 w-full h-screen overflow-hidden flex items-center justify-center perspective-2000">
        
        {/* Loading Overlay */}
        {!loaded && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gray-50 dark:bg-[#020202]">
            <div className="text-blue-500 font-mono text-[10px] uppercase tracking-[0.5em] mb-4">
              Syncing Core Frames... {loadingProgress}%
            </div>
            <div className="w-1/4 h-[1px] bg-black/10 dark:bg-white/10 overflow-hidden">
               <motion.div 
        ...
        >
          <canvas
            ref={canvasRef}
            className="w-full h-full object-cover dark:invert-0 invert grayscale dark:grayscale-0 mix-blend-multiply dark:mix-blend-normal"
          />
        </motion.div>
      ...
      >
        <h3 className="text-black dark:text-white text-4xl md:text-8xl font-black uppercase tracking-tighter mb-2 drop-shadow-2xl">
